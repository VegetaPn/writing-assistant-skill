...
3. **热点分析信息源限定**: 热点分析/监控爆款时，信息源**仅限 X/Twitter 和小红书**，不得自由发挥使用 WebSearch 或其他平台。唯一例外：当背景了解不足或上下文不够理解某个话题时，可以搜索**事实性信息**作为上下文补充（如某个事件的背景、某个概念的含义），但这不是热点信息源，不能用于替代 X/小红书的热点数据。
4. **bird CLI 超时处理**: bird CLI 超时通常是因为 X/Twitter 在中国境内无法直连。此时应询问用户是否需要配置代理，默认代理地址为 `127.0.0.1:7890`。配置方式：在 bird 命令前加 `HTTPS_PROXY=http://127.0.0.1:7890`（或用户提供的地址）。
5. **图片生成模型强制**: 使用 generate-image 生成图片时，**必须使用 `google/gemini-3.1-pro-preview`**，禁止私自选择 `black-forest-labs/flux.2-pro` 或其他模型。仅当用户明确指定其他模型时才可更改。
