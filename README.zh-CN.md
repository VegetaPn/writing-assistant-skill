...
一个 Claude Code 技能，编排从构思到发布的端到端写作流程——自动生成配图、内置选题管理、爆款对标、经验沉淀、多平台发布、**自主模式**实现全程无人值守执行，以及**定时任务**自动化周期性工作流。
...
想要发完指令就去睡觉，回来看到成品？直接说：

```
自主模式，帮我写一篇关于注意力经济的小红书文章
```

AI 全程自动执行完整工作流——不需要任何交互。所有决策自主完成并记录，只有当严格的 Completion Gate 清单全部通过后才会停止。

更多示例：

```
全自动写一篇公众号文章，主题是注意力经济，写完发到公众号
autonomous mode, write three XHS articles on different angles of AI productivity
...
两种执行模式：**交互模式**（默认，逐步与用户协作）或**自主模式**（全程无人值守）。

### 定时任务

| 命令 | 说明 |
|------|------|
| `每天早上9点监控爆款` | 创建周期性定时任务 |
| `先监控爆款，然后深化选题` | 创建多步流水线 |
| `查看定时` / `看计划` | 查看所有定时任务 |
| `执行历史` / `看历史` | 查看历史执行记录 |
| `暂停定时 cron-001` | 暂停一个定时任务 |
| `恢复定时 cron-001` | 恢复已暂停的任务 |
| `取消定时 cron-001` | 删除一个定时任务 |
...
**自主模式（2.1 新增）**
- 发一条指令，去睡觉，回来看成品
- AI 全程自主决策——平台、标题、结构、风格——每个决策都记录在案
- 失败不停：工具失败记录并跳过，绝不阻塞流程
- 卡住检测：自动识别并跳过卡死的操作
- Completion Gate：严格的 7 项停止条件清单（G1-G7），全部通过才能停止——保证不会半途而废
- 支持批量执行："把 inbox 里的选题都写了"、"写三篇不同角度的文章"
- 完全透明：Autonomous Decision Log + 执行摘要，事后可完整审阅
- 自主模式默认跳过配图（加速执行，如需配图在指令中说明）

**定时任务（2.2 新增）**
- 自然语言驱动："每天早上9点监控爆款" → 自动生成 crontab 定时任务
- 两种任务形态：**单任务**（一次 cron 触发 → 一次执行）和**流水线**（一次 cron 触发 → 多步串行执行，步骤间共享上下文）
- 流水线上下文感知：每步读取 `pipeline-context.md`，包含整体目标和已完成步骤的输出，实现步骤间自然的信息流转
- 全生命周期管理：创建、查看、暂停、恢复、修改、删除
- 执行历史：实时日志、逐步输出记录、系统通知
- 并发控制：锁文件机制防止重叠执行
- 结束条件：无限循环、次数限制（`count:N`）、日期限制（`until:YYYY-MM-DD`）
- 一次性任务执行后自动清理
- 通过 `claude CLI` + `--dangerously-skip-permissions` 实现无人值守执行
...
├── scripts/                       # 自动化脚本
│   ├── check-env.sh               # 环境预检
│   ├── cron-runner.sh             # 任务执行引擎
│   ├── install-schedule.sh        # 注册 crontab 条目
│   └── uninstall-schedule.sh      # 移除 crontab 条目
├── schedules/                     # 定时任务数据
...
定时任务——用自然语言调度自动化周期性工作流。

- Scheduler 子技能：自然语言驱动的定时任务调度（"每天早上9点监控爆款" → crontab）
- 两种任务形态：单任务和多步流水线（步骤间共享上下文）
- 流水线上下文感知：`pipeline-context.md` 在步骤间传递目标、摘要和输出
- 全生命周期管理：创建、查看、暂停、恢复、修改、删除
- 执行引擎（`cron-runner.sh`）：验证、加锁、超时、流式处理、历史记录
- 实时执行日志：工具调用实时追踪
- 长任务心跳通知
- 并发控制：锁文件机制防止重叠执行
- 结束条件：无限循环、次数限制、日期限制，到期自动清理
- 一次性任务执行后自动移除 crontab 条目
- 系统通知（macOS/Linux）：任务完成或失败时推送
- 执行历史：逐次输出文件、流水线摘要、步骤级详情

### 2.1.0 (2026-02-22)

自主模式——发一条指令，去睡觉，回来看成品。

- 自主模式：从构思到成品全程无人值守执行
- 自然语言完成条件（"写三篇文章"、"把 inbox 都写了"等）
- Completion Gate：强制 7 项停止条件清单（G1-G7），全部通过才能停止
- Autonomous Decision Log：每个 AI 决策记录选择、理由和备选方案
- 失败不停：工具失败记录并跳过，绝不阻塞
- 卡住检测：自动识别并跳过卡死的操作
- 自主能力边界：需用户参与的操作（登录、扫码等）自动跳过并标注
- 自主模式默认跳过配图（加速执行，可选开启）
- 批量执行：多篇文章独立 progress tracker，跨篇经验自动传递
- 执行摘要：结构化的事后报告（关键决策、产出文件、失败项、建议复查点）
