...
| # | 阶段 | 解决的痛点 | 你的决定 |
|---|------|-----------|---------|
| 2 | 素材沉淀系统 | #2 每次从零创作，无法复用 | |
| 3 | 中文参考库 + 去 AI 味 | #5 参考库只有英文，缺中文平台规范 | |
| 4 | Skill 拆分与细化 | #6 只有 1 个大 skill，无法灵活调用 | |
| 5 | 选题管理系统 | #7 碎片想法无处记录，无生命周期 | |
| 6 | 多平台适配 | #8 无法一鱼多吃 | |
| 7 | 爆款对标系统 | #3 不知道什么内容容易火 | |
| 8 | 数据闭环系统 | #4 发布后无数据反馈 | |
...
| 项目 | 说明 | 你的决定 |
|------|------|---------|
| 半自动抓取（方式 B） | 自动抓取作者公开内容并归档 | |
| 增加更多作者和文章 | 如 dontbesilent（素材已就绪） | |
| 标题/开头的数据标注 | 给参考库内容标注点击率、完播率等 | |
| 参考库搜索和过滤 | 提升检索效率 | |
